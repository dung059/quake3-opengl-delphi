...
begin
  ...
  for I := 0 to FQ3_base_Path.Count - 1 do
  begin
    fullpath := IncludeTrailingBackslash(FQ3_base_Path.Strings[i]) + 'scripts\';

    dir := FQ3_base_Path.Strings[i] + 'scripts\*.shader';
  ...
  for I := 0 to fPk3Zip.Count- 1 do
    for k := 0 to fPk3Zip.Items[i].Count - 1 do
      begin
    ...
    //        [xfile, ExtractFileExt(xfile), fPk3Zip.Items[i].FullPath,
    //        formatdatetime('c', now())]));
          result := FAvailableShaders.Count <> 0;
        end;
      end;
...
constructor TShaderManager.Create(vPk3Zip: TZipPK3);
var
  i, j: Integer;
  value, name: string;
  me: TMemoryStream;
...
begin
  ...
  FAvailableShaders.Sorted := true;
  FAvailableShaders.Duplicates := dupIgnore;
  FRequiredShaders := TStringList.Create;
  FRequiredShaders.Sorted := true;
  FRequiredShaders.Duplicates := dupIgnore;
  ...
  if not Assigned(vPk3Zip) then
    ...
    FPk3Zip := vPk3Zip;
  FQ3_base_Path := TStringList.Create;
  FQ3_base_Path.Assign(FPk3Zip.BASE_PATH);
    ...
    with FPk3Zip.Items[FPk3Zip.IsOpenIndex] do
      begin
        for I := 0 to Count - 1 do
           if FileNames[i].Ext = '.shader' then
              begin
                me := FPk3Zip.GetFileBytes(i);
                ScanShader('', FileNames[i].Full, me);
              end;
      end;
end;
...
begin
  ...
  if Assigned(FQ3_base_Path) then
    FQ3_base_Path.Free;
    ...
    begin
      ...
      begin
        ...
        if (line <> '') and (stage=0) and (line <> '{') then begin // must be the shader name
          ...
          if FAvailableShaders.IndexOf(tmp) = -1 then
...
begin
  ...
  k := 0;
  while k < FQ3_base_Path.Count do
  begin
    path := FQ3_base_Path.Strings[k];
    inc(k);
...
function TShaderManager.ReadShadersFromList(filename : string; list : TStringList;
