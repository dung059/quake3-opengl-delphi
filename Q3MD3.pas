...
uses
  Windows, SysUtils, OpenGL, Textures, System.Classes;

type
  TMD3Header = Record
    ID : Array[1..4] of AnsiChar;               // id = IDP3
    Version       : Integer;                // Version = 15
    Filename      : Array[1..68] of AnsiChar;
  ...
  TBoneFrame = Record
    ...
    Scale    : glFloat;
    Creator  : Array[1..16] of AnsiChar;
  end;
  ...
  TMesh = Record
    ...
    Skins      : Array of Array[1..68] of AnsiChar;
  ...
  TMD3Model = object
    ...
    procedure LoadModel(memory : TMemoryStream; filename : String); overload;
    procedure LoadModel(filename : String); overload;
    ...
    procedure LoadSkin(Imagepath, filename : String; memory : TMemoryStream);
  end;
  ...
  Q3Player = Object
    ...
    procedure LoadAnim(filename : String; memory : TMemoryStream);
...
function CharArrToStr(const C : Array of AnsiChar) : String;
...
{---------------------------------------------------------}
procedure TMD3Model.LoadModel(memory: TMemoryStream; filename : String);
var
    I : Integer;
    MeshOffset : Integer;
begin
  ...
  // read boneframes
  SetLength(BoneFrames, Header.numBoneFrames);
  memory.ReadBuffer(BoneFrames[0], Header.numBoneFrames*sizeof(TBoneFrame));

  // read tags
  SetLength(Tags, Header.numBoneFrames * Header.numTags);
  memory.ReadBuffer(Tags[0], Header.numBoneFrames*Header.numTags*sizeof(TTag));
  ...
  // read meshes
  SetLength(Meshes, Header.numMeshes);
  MeshOffset := memory.Position;

  For I :=0 to Header.numMeshes-1 do
  begin
    memory.Seek(MeshOffset, soFromBeginning);
    memory.ReadBuffer(Meshes[I].MeshHeader, sizeOf(TMeshHeader));

    // Load the Skins
    SetLength(Meshes[I].Skins, Meshes[I].MeshHeader.numSkins);
    memory.ReadBuffer(Meshes[I].Skins[0], 68 * Meshes[I].MeshHeader.numSkins);

    // Triangles
    memory.Seek(MeshOffset + Meshes[I].MeshHeader.triStart, soFromBeginning);
    SetLength(Meshes[I].Triangle, Meshes[I].MeshHeader.numTriangles);
    memory.ReadBuffer(Meshes[I].Triangle[0], sizeOf(TTriangle)*Meshes[I].MeshHeader.numTriangles);

    // Texture Coordiantes
    memory.Seek(MeshOffset + Meshes[I].MeshHeader.TexVectorStart, soFromBeginning);
    SetLength(Meshes[I].TexCoord, Meshes[I].MeshHeader.numVertexes);
    memory.ReadBuffer(Meshes[I].TexCoord[0], sizeOf(TTexCoord)*Meshes[I].MeshHeader.numVertexes);

    // Vertices
    memory.Seek(MeshOffset + Meshes[I].MeshHeader.VertexStart, soFromBeginning);
    SetLength(Meshes[I].Vertex, Meshes[I].MeshHeader.numVertexes * Meshes[I].MeshHeader.numMeshFrames);
    memory.ReadBuffer(Meshes[I].Vertex[0], sizeOf(TVertex)*Meshes[I].MeshHeader.numVertexes * Meshes[I].MeshHeader.numMeshFrames);
...
procedure TMD3Model.LoadSkin(Imagepath, filename : String; memory : TMemoryStream);
var F : TextFile;
    I, k : Integer;
    S : String;
...
begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  LoadTexture(ImagePath + ImageName, meshes[i].Texture, FALSE, false);
  ...
  if memory.Size > 0 then
  begin
    SetString(line, PAnsiChar(memory.Memory), memory.Size);
    memory.Free;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if UpperCase(CharArrToStr(meshes[i].MeshHeader.Name)) = Uppercase(meshname) then
                begin
                  LoadTexture(ImagePath + ImageName, meshes[i].Texture, FALSE, true);
                  meshes[i].SetTexture :=TRUE;
                end;
              end;
            end;
...
{---------------------------------------------------------}
procedure Q3Player.LoadAnim(filename: String; memory : TMemoryStream);
var F : Text;
    S : String;
    I, P, skip, k: Integer;
    line : string;
...
begin
  ...
  Lower.LoadSkin(path, 'lower_' + skin + '.skin', Pk3Zip.GetFileBytes('lower_' + skin + '.skin'));
  Upper.LoadSkin(path, 'upper_' + skin + '.skin', Pk3Zip.GetFileBytes('upper_' + skin + '.skin'));
  Head.LoadSkin(path, 'head_' + skin + '.skin', Pk3Zip.GetFileBytes('head_' + skin + '.skin'));

  LoadAnim(path + 'animation.cfg', Pk3Zip.GetFileBytes('animation.cfg'));
